...
cv.function.gaussian <- function(features, labels, K, l, reweight = FALSE){
  ### Input:
  ...
  for (i in 1:K){
    ...
    ## model training
    if (reweight){
      model_train <- train_gaussian(feature_train, label_train, w = weight_train, l)
    } else {
      model_train <- train_gaussian(feature_train, label_train, w = NULL, l)
    }
