...
if(run.cv.randomForest){
  ntree_best <- res_cv_randomForest$ntree[which.min(res_cv_randomForest$mean_error)]
}
save(ntree_best,file = "../output/ntree_best_randomForest.Rdata")
cat("ntree_best=", ntree_best)
```
...
##Training
tm_train=NA
tm_train <- system.time(fit_train_randomForest <- train_randomForest(feature_train, label_train, ntree=ntree_best))
...
accu <- mean(test_label == label_pred_randomForest)
tpr.fpr <- WeightedROC(as.numeric(label_pred_randomForest), test_label)
auc <- WeightedAUC(tpr.fpr)

cat("The accuracy of model:", randomForest_model_labels[which.min(res_cv_randomForest$mean_error)], "is", accu*100, "%.\n")
cat("The AUC of model:", randomForest_model_labels[which.min(res_cv_randomForest$mean_error)], "is", auc, ".\n")
```
