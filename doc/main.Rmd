...
if (run.svm.polynomial.prob.cv.test) {
  acc.array <- svm_polynomial_prob_best_degree(dat_train, 5) # up to degree 5
  max.deg <- which.max(acc.array)
} else {
  max.deg = 4
}
...
if (run.train.svm.polynomial) {
  ...
  save(sclassifier.polynomial, file="../output/svmclassifier_polynomial_prob.RData")
} else{ 
...
```{r}
testset <- as.matrix(dat_test[, -6007])
ypred_prob <- predict(sclassifier.polynomial, testset, decision.values = TRUE, probability = TRUE)
```
