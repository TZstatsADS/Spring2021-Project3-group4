...
if (run.train.svm.polynomial) {
  ...
  save(sclassifier.polynomial, file="svmclassifier_polynomial.RData")
} else{ 
  load(file="")
}
```
...
if(run.cv){
  ...
  for(i in 1:length(lmbd)){
    cat("lambda = ", lmbd[i], "\n")
    xgb_res_cv[i,] <- xgbcv.function(features = feature_train, labels = label_train, K,eta_val =0.05,
                                     lmd = lmbd[i],gamma = 0.1,md = 5,nr = 500,  reweight = sample.reweight)
...
if (sample.reweight){
  xgb_tm_train <- system.time(fit_train <- xgbtrain(feature_train, label_train, w = weight_train, eta_val = 0.05 ,gamma = 0.1, lmd = par_best, nr = 500, md = 5))
} else {
  xgb_tm_train <- system.time(fit_train <- xgbtrain(feature_train, label_train, w = NULL, eta_val = 0.05 ,gamma = 0.1, lmd = par_best, nr = 500, md = 5))
}
...
cat("The AUC of model:", model_labels[which.max(xgb_res_cv$mean_AUC)], "is", auc, ".\n")
