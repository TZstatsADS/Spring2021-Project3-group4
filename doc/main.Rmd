...
if (run.train.xgb){
  xgb_tm_train <- system.time(fit_train <- xgbtrain(feature_train_balanced, label_train_balanced, w = NULL, eta_val = 0.08 ,gamma = 0.4, lmd = par_best, nr = 200, md = 5))
...
```{r pca_plot}
plot(pca.list, pc, type="l", xlab="number of PCs", ylab="mean error")
```
...
```{r get_pc}
# since the data file is too large, we are unable to upload it on Github, set run.pca = TRUE to get the data, but note that it takes time
if(run.pca) {
  #training dataset
