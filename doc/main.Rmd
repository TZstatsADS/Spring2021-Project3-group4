...
cat("The accuracy of model:", model_labels[which.max(xgb_res_cv$mean_AUC)], "is", accu_xgboost*100*100, "%.\n")
...
```{r}
source("../lib/cross_validation_enet.R")
source("../lib/train_gaussian.R")
```
...
label_train = as.integer(dat_train$label) 
if(run.cv.enet){
  res_cv_gaussian <- matrix(0, nrow = length(lmbd), ncol = 4)
  for(i in 1:length(lmbd)){
    cat("lambda = ", lmbd[i], "\n")
    res_cv_gaussian[i,] <- cv.function.gaussian(features = feature_train, labels = label_train, K, 
                              l = lmbd[i], reweight = sample.reweight)
  save(res_cv_gaussian, file="../output/res_cv_gaussian.RData")
  }
}else{
  load("../output/res_cv_gaussian.RData")
}
res_cv_g <- as.data.frame(res_cv_gaussian) 
colnames(res_cv_g) <- c("mean_error", "sd_error", "mean_AUC", "sd_AUC")
res_cv_g$k = as.factor(lmbd)

par_best_g <- lmbd[which.min(res_cv_g$mean_error)] # equals 0.05
```
...
if (sample.reweight){
  tm_train_enet <- system.time(fit_train_enet <- train_gaussian(feature_train, label_train, w = weight_train, par_best_g ))
} else {
  tm_train_enet <- system.time(fit_train_enet <- train_gaussian(feature_train, label_train, w = NULL, par_best_g ))
}
```
...
if(run.test.enet){
  ...
  tm_test_enet <- system.time({label_pred_enet <- as.integer(test(fit_train_enet, feature_test, pred.type = 'class')); 
                          prob_pred_enet <- test(fit_train_enet, feature_test, pred.type = 'response')})
}
```
...
accu <- sum(weight_test * (label_pred_enet == label_test)) / sum(weight_test)
tpr.fpr <- WeightedROC(prob_pred_enet, label_test, weight_test)
auc <- WeightedAUC(tpr.fpr)
cat("The accuracy of model:", model_labels[which.min(res_cv_g$mean_error)], "is", accu*100, "%.\n")
cat("The AUC of model:", model_labels[which.min(res_cv_g$mean_error)], "is", auc, ".\n")
```
...
label_train = as.integer(dat_train$label)
if(run.cv.knn){
  res_cv <- matrix(0, nrow = length(k), ncol = 4)
  for(i in 1:length(k)){
    cat("k=", k[i], "\n")
    res_cv_knn[i,] <- cv.function(features = feature_train, labels = label_train, K, 
                              k[i], reweight = sample.reweight)
  save(res_cv, file="../output/res_cv_knn.RData")
  }
}
```
...
```{r best_model}
if(run.cv){
  par_best <- k[which.min(res_cv_knn$mean_error)] # k[which.max(res_cv_knn$mean_AUC)]
}
```
...
tpr.fpr <- WeightedROC(prob_pred, label_test, weight_test)
auc <- WeightedAUC(tpr.fpr)
cat("The accuracy of the knn model:", model_labels[which.min(res_cv_knn$mean_error)], "is", accu*100, "%.\n")
cat("The AUC of the knn model:", model_labels[which.min(res_cv_knn$mean_error)], "is", auc, ".\n")
```
